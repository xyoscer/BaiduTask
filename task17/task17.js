...
/**
   ...
   （3）使用innerHTML给页面实时添加渲染的内容图表
    ...
    （7）使用new Date(data)).getDay() == 6来某一天是周几，如果是6,就说明此时是日历上的完整一周
    （8）使用getMonth()来统计月份
...
// 用于渲染图表的数据 时间(2016-01-01)：数量(255)
var chartData = {};
// 记录当前页面的表单选项
var pageState = {
  nowSelectCity: "北京",
...
function initAqiChartData() {
  ...
  //nowCityData 表示当前城市的某天的降水量：2016-01-01:255
  ...
  if (pageState.nowGraTime == 'week') {
    ...
    for (var data in nowCityData) {
      countSum += nowCityData[data];
      daySum ++;
      if ((new Date(data)).getDay() == 6 ) {
        week ++;
  ...
  if (pageState.nowGraTime == 'month') {
    ...
    for (var data in nowCityData) {
      countSum += nowCityData[data];
      daySum ++;
      if ((new Date(data)).getMonth() !== month) {
        month ++;
